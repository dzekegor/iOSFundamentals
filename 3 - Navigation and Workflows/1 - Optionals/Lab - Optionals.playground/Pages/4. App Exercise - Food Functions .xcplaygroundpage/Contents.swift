...
func getMeal(course: String) -> Meal? {
    if meals[course] != nil {
        return meals[course]
    } else {
        return nil
...
print(getMeal(course: "Breakfast"))
print(getMeal(course: "Dinner"))
/*:
 ...
 */
func getMealLog(mealLog: [String: Meal]) -> [String: Meal]? {
    if let log = UserDefaults.standard.dictionary(forKey: "mealLog") {
        return log as! [String: Meal]
    } else {
        return nil
    }
}
print(getMealLog(mealLog: meals))
/*:
